...
//        L_endeffector = L_endeffector + 1e-2 * MatrixXd::Identity(L_endeffector.rows(), L_endeffector.cols());
    ...
    RCLCPP_INFO_THROTTLE(
        ...
        *rclcpp::Clock::make_shared(), 
        ...
        "forward kinematics calculated for %lf s", 
        integration_time
    );
...
int Kinematics::GetJointIdByName(string joint) {
    ...
    if (joint_index == iDynTree::JOINT_INVALID_INDEX) {
        // RCLCPP_INFO_THROTTLE(rclcpp::get_logger("rclcpp"), std::chrono::seconds(5), "joint " << joint.c_str() <<" not found in model");
        RCLCPP_INFO_THROTTLE(
        rclcpp::get_logger("rclcpp"), 
        *rclcpp::Clock::make_shared(), 
...
bool Kinematics::parseViapoints(const string &viapoints_file_path, vector<Cable> &cables) {
    ...
    if (!doc.LoadFile()) {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger("rclcpp"), "Can't parse via points file " << viapoints_file_path.c_str() << ".");
        return false;
    }
