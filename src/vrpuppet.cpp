...
//    robotstate.world_H_base.setRotation(rot);
    ...
    // ros control
    for (int joint = 0; joint < number_of_dofs; joint++) {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"initializing controllers for joint %d %s", joint, joint_names[joint].c_str());
    ...
    for (string ef:endeffectors) {
        ...
        if (!ok) {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("rclcpp"), "Oh no!: impossible to load model from " << urdf_file_path);
            return;
        }
    ...
    floating_base_sub = node_->create_subscription<geometry_msgs::msg::Pose>(
    ...
    ik_srv = node_->create_service<roboy_middleware_msgs::srv::InverseKinematics>(
    ...
    link_pose_srv = node_->create_service<roboy_control_msgs::srv::GetLinkPose>(
      "/get_link_pose", std::bind(&Robot::GetLinkPoseService, this, std::placeholders::_1, std::placeholders::_2));  

    //??? visualization_msgs
    // interactive_marker_sub = nh->subscribe("/interactive_markers/feedback",1,&Robot::InteractiveMa   rkerFeedback, this);
    // interactive_marker_sub = node_->create_subscription<visualization_msgs::msg::InteractiveMarkerFeedbackConstPtr>(
    ...
    if (first_time_solving) {
        ...
        switch (status) {
            case SUCCESSFUL_RETURN: { // all good
                ...
                log_stream << "target cable forces:\n" << f_opt.transpose().format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "\n", "[", "]\n"));
                RCLCPP_INFO_THROTTLE(rclcpp::get_logger("rclcpp"), *rclcpp::Clock::make_shared(), 1 * 1000, log_stream.str());
                break;
            }
            default: {
                // ROS_ERROR_STREAM_THROTTLE(1, MessageHandling::getErrorCodeMessage(status));
                
                RCLCPP_ERROR_THROTTLE(rclcpp::get_logger("rclcpp"), *rclcpp::Clock::make_shared(), 1*1000, MessageHandling::getErrorCodeMessage(status));
                if (qp_solver.isInfeasible()) {
                    ...
                    std::stringstream log_stream;
                    log_stream << "target cable forces:\n" << f_opt.transpose().format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "\n", "[", "]\n"));
                    RCLCPP_WARN_THROTTLE(rclcpp::get_logger("rclcpp"), *rclcpp::Clock::make_shared(), 1 * 1000, log_stream.str());
//                    f_opt.setZero();
                } else {
...
std::stringstream log_stream;
log_stream << "q " << q.transpose().format(fmt);
RCLCPP_INFO_THROTTLE(rclcpp::get_logger("rclcpp"), *rclcpp::Clock::make_shared(), 5 * 1000, log_stream.str());
...
bool Robot::ForwardKinematicsService(roboy_middleware_msgs::srv::ForwardKinematics::Request::SharedPtr req,
                                     roboy_middleware_msgs::srv::ForwardKinematics::Response::SharedPtr res) {
    if (ik_models.find(req->endeffector) == ik_models.end()) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("rclcpp"),"endeffector " << req->endeffector << " not initialized");
        return false;
    }
    ...
    for (string joint:req->joint_names) {
        ...
        if (joint_index != iDynTree::JOINT_INVALID_INDEX) {
            ...
            RCLCPP_WARN_THROTTLE(rclcpp::get_logger("rclcpp"), *rclcpp::Clock::make_shared(), 5.0 * 1000, "joint %s not found in model", joint.c_str());
    ...
    tf2::convert(iso, res->pose);
    return true;
}

bool Robot::InverseKinematicsService(roboy_middleware_msgs::srv::InverseKinematics::Request::SharedPtr req,
                                     roboy_middleware_msgs::srv::InverseKinematics::Response::SharedPtr res) {
    if (ik_models.find(req->endeffector) == ik_models.end()) {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("rclcpp"),"endeffector " << req->endeffector << " not initialized");
        return false;
    }
...
//    ik[req.endeffector].setCostTolerance(0.0001);
    ...
    */
    ...
    // cannot find definition to COLOR and publishMesh
    // COLOR color(1,1,1,1);
    // color.randColor();
...
//                                         << q_star.toString());
        ...
        switch (req->type) {
            case 0:
                RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"unable to solve full pose ik for endeffector %s and target frame %s:\n"
                ...
                break;
            case 1:
                RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"unable to solve position ik for endeffector %s and target frame %s:\n"
                          "target_position    %.3lf %.3lf %.3lf", req->endeffector.c_str(), req->target_frame.c_str(),
                          req->pose.position.x, req->pose.position.y, req->pose.position.z);
                break;

            case 2:
                RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"unable to solve orientation ik for endeffector %s and target frame %s:\n"
                          "target_orientation %.3lf %.3lf %.3lf %.3lf", req->endeffector.c_str(), req->target_frame.c_str(),
                          req->pose.orientation.w, req->pose.orientation.x, req->pose.orientation.y,
                          req->pose.orientation.z);
                break;
...
bool Robot::ExecuteIK(roboy_middleware_msgs::srv::InverseKinematics::Request::SharedPtr req,
                        roboy_middleware_msgs::srv::InverseKinematics::Response::SharedPtr res) {
...
bool Robot::GetLinkPoseService(roboy_control_msgs::srv::GetLinkPose::Request::SharedPtr req, 
                                    roboy_control_msgs::srv::GetLinkPose::Response::SharedPtr res) {
    for (int i = 0; i < number_of_links; i++) {
...
// }
void Robot::FloatingBase(const geometry_msgs::msg::Pose::SharedPtr msg) {
    Isometry3d iso;
    //tf::poseMsgToEigen(*msg, iso);
    tf2::convert(*msg, iso);
    world_H_base = iso.matrix();
...
//     roboy_control_msgs::action::MoveEndEffector_Feedback feedback;
//     roboy_control_msgs::action::MoveEndEffector_Result result;
//     bool success = true, timeout = false;

//     double error = 10000;
//     rclcpp::Time last_feedback_time = rclcpp::Clock().now(), start_time = rclcpp::Clock().now();
//     bool ik_solution_available = false;

//     auto it = find(endeffectors.begin(),endeffectors.end(),goal->endeffector);
//     if (it == endeffectors.end()) {
//         RCLCPP_ERROR_STREAM(rclcpp::get_logger("rclcpp"),"MoveEndEffector: FAILED endeffector " << goal->endeffector << " does not exist");
...
bool Robot::parseViapoints(const string &viapoints_file_path, vector<Cable> &cables) {
    ...
    if (!doc.LoadFile()) {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger("rclcpp"), "Can't parse via points file " << viapoints_file_path << ".");
        return false;
    }
            ...
            for (link_child_it = myoMuscle_it->FirstChildElement("link"); link_child_it;
                ...
                if (!link_name.empty()) {
                    ...
                    if (cable.viaPoints.empty()) {
                        RCLCPP_ERROR_STREAM(rclcpp::get_logger("parser"), "No viaPoint element found in myoMuscle '" << cable.name << "' link element.");
